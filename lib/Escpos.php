...
class Escpos
{
	...
	public function print_invoice($data) {
		...
		if(!empty($data->commission_agent)){
			$commission_agent = $data->commission_agent_label;
			$commission_agent .= ': ' . $data->commission_agent;
			$this->printer->text($commission_agent);
			$this->printer->feed(1);
		}

		// Sales person
		// if(!empty($data->sale_person)){
		// 	$sale_person = $data->sales_person_label;
		// 	$sale_person .= ': ' . $data->sale_person;
		// 	$this->printer->text($sale_person);
		// 	$this->printer->feed(1);
		// }
		...
		if (isset($data->stripe_charges) && !empty($data->stripe_charges)) {
			$stripe_charges = $this->columnify($data->stripe_charges_label, $data->stripe_charges, 50, 50, 0,0);
			// $tax = $this->columnify('', $data->tax_label . ' ' . $data->tax, 40, 60, 0,0);
			$this->printer->text(rtrim($stripe_charges));
			$this->printer->feed();
		}
